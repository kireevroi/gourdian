...
func itemDName(items map[string]dotadata.ItemInfo, name string) string {
...
func partsFacts(held []string, items map[string]dotadata.ItemInfo, b *dotadata.Build, clock int) []PartsFact {
	if b == nil || items == nil {
		return nil
	}
	...
	for i, it := range b.Items {
		...
		info := items[it.Name]
		f := PartsFact{Item: info.DName, Left: dotadata.RemainingCost(it.Name, held, items)}
		recipe := info.Cost
