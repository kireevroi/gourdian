...
func TestNoSpendGoldAlertWhileSavingForTheNextItem(t *testing.T) {
	...
	if got := byRule(play(newEngine(data), settings(config.RoleCarry), 900, 1000, holding(2767)), "unspent_gold"); len(got) != 0 {
		t.Fatalf("saving for Black King Bar isn't unspent gold: %+v", got)
	}
	if got := byRule(play(newEngine(data), settings(config.RoleCarry), 900, 1000, holding(6000)), "unspent_gold"); len(got) != 1 {
