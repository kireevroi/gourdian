...
func TestPowerRunesOnScheduleOnlyWhileLaning(t *testing.T) {
	var at []int
	for _, tip := range byRule(play(newEngine(nil), settings(dota.Mid), 300, 1500, nil), "power_runes") {
...
func TestPowerRunesAfterLaningOnlyWhenNearOne(t *testing.T) {
	far := byRule(play(newEngine(nil), settings(dota.Carry), 700, 900, nil), "power_runes_near")
	if len(far) != 0 {
		t.Fatalf("far from both rune spots: %+v", far)
	}
	nearTop := func(s *gsi.State) { s.Hero.XPos, s.Hero.YPos = -2600, 1800 }
	got := byRule(play(newEngine(nil), settings(dota.Carry), 700, 900, nearTop), "power_runes_near")
