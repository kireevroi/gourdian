...
func warm(t *testing.T, od *opendota.Client, path, role string) {
	hero := 0
...
func askAt(t *testing.T, e *coach.Engine, data *opendota.Client, set config.Settings, hero *gsi.Hero) {
	...
	facts := &prompts.HeroFacts{Build: map[string][]string{}}
	...
	snap := e.Snapshot(set)
	prompt := prompts.Prompt(prompts.Input{Context: prompts.Context{Hero: facts}, Reason: "regular check-in", Role: set.Role,
	...
	for _, p := range ai.NewProviders(env) {
		...
		defer cancel()
		tips, err := prompts.Suggest(ctx, p, config.AIChoice{Provider: "claude", Model: "sonnet", Effort: "low"}, config.AISettings{}, os.Getenv("LANGUAGE_CODE"), prompt)
