...
func TestLoggedOutClaudePausesCoachAndWarnsOnce(t *testing.T) {
	...
	if srv.providers.Ready() || srv.providers.HealthOf("claude").Problem != connect.LoggedOut || srv.providers.Banner().Provider != "claude" {
...
func TestPersonalTargetsFromHistory(t *testing.T) {
	...
	openDotaAPI := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
