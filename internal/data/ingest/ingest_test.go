...
func newService(t *testing.T) Service {
	t.Helper()
	parsed, err := os.ReadFile("../opendota/testdata/match_parsed.json")
	if err != nil {
		t.Fatal(err)
	}
...
func TestRoleFor(t *testing.T) {
	cases := []struct {
		...
		{opendota.PlayerDetail{LaneRole: opendota.LaneMid, NetWorthRank: 4}, dota.Mid},
		{opendota.PlayerDetail{LaneRole: opendota.LaneSafe, NetWorthRank: 1}, dota.Carry},
		{opendota.PlayerDetail{LaneRole: opendota.LaneSafe, NetWorthRank: 5}, dota.HardSupport},
		{opendota.PlayerDetail{LaneRole: opendota.LaneOff, NetWorthRank: 2}, dota.Offlane},
		{opendota.PlayerDetail{LaneRole: opendota.LaneOff, NetWorthRank: 4}, dota.SoftSupport},
		{opendota.PlayerDetail{LaneRole: opendota.LaneJungle, Roaming: true, NetWorthRank: 5}, dota.SoftSupport},
	}
	for _, c := range cases {
...
func TestItemTimingsSkipCombinedComponents(t *testing.T) {
	...
	d := opendota.PlayerDetail{ItemTimes: map[string]int{"sange": 900, "yasha": 1000, "sange_and_yasha": 1010, "black_king_bar": 700}}
	var got []string
