...
type fakeBuilds struct {
	...
	timings map[string][]dotadata.ItemTiming
}

func (f *fakeBuilds) Items() map[string]dotadata.ItemInfo { return f.items }

func (f *fakeBuilds) BuildFor(int, string) *dotadata.Build { return f.build }

func (f *fakeBuilds) ItemTimings(_ int, item string) ([]dotadata.ItemTiming, bool) {
	b, ok := f.timings[item]
	return b, ok
}

func catalogue() map[string]dotadata.ItemInfo {
	return map[string]dotadata.ItemInfo{
...
func TestALoadingBuildIsRetriedButACompleteOneIsKept(t *testing.T) {
	...
	builds.build = &dotadata.Build{Items: []dotadata.BuildItem{{Name: "manta", Cost: 4650, Phase: dotadata.PhaseMid}}}
	builds.timings = map[string][]dotadata.ItemTiming{"manta": {{Time: 1500, Games: 40, Wins: 24}}}
	h.version = 2
	c.TargetsFor(1, dota.Carry)
	before := h.asked
	age(c)
