...
type BuildSource interface {
	...
	ItemTimings(heroID int, item string) ([]opendota.ItemTiming, bool)
}
...
func (c *Cache) itemGoals(heroID int, role string, history []model.MatchSummary) ([]coach.ItemGoal, int, bool) {
	...
	if len(chosen) == 0 {
		...
		for _, it := range build.Items {
			if (it.Phase == opendota.PhaseMid || it.Phase == opendota.PhaseLate) && it.Cost >= dota.GoalItemCost {
	...
	for _, name := range chosen {
		...
		good, hasGood := opendota.GoodTiming(buckets)
		usual := 0
		if len(times[name]) >= 3 {
