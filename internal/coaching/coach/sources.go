...
func buildSource(b *opendota.Build, hero, role string, w sourceWords) Source {
	...
	case b.Games == 0:
		...
		if pos > 0 && !b.Loading {
			notes = append(notes, w.f("Pros won fewer than %d games with it in the last %d days.",
				"За %[2]d дней про-игроки выиграли на нём меньше %[1]d матчей.", opendota.MinProGames, opendota.ProDays))
		}
	case b.Won:
		src.From = w.f("OpenDota: what pros bought on %s %s in the %d games they won over the last %d days.",
			"OpenDota: покупки про-игроков на %[1]s%[2]s в выигранных матчах за последние %[4]d дней, матчей: %[3]d.", hero, where, b.Games, opendota.ProDays)
	...
	default:
		src.From = w.f("OpenDota: what pros bought on %s %s in all %d of their games there over the last %d days, won or lost.",
			"OpenDota: покупки про-игроков на %[1]s%[2]s во всех их матчах за последние %[4]d дней, победы и поражения, матчей: %[3]d.", hero, where, b.Games, opendota.ProDays)
		src.Short = w.f("%s, %d pro games", "%s, про-матчей: %d", short, b.Games)
	}
	switch {
	case b.Position == 0 && b.Games > 0 && pos > 0 && !b.Loading:
		notes = append(notes, w.f("Pros played it as %s in fewer than %d games.", "На позиции «%s» у про-игроков меньше %d матчей.", dota.RoleName(role, w.lang), opendota.MinProGames))
	case b.Position > 0 && !b.Won:
		notes = append(notes, w.f("They won fewer than %d of them, too few for a build from wins alone.",
			"Побед среди них меньше %d — слишком мало для сборки только по победам.", opendota.MinProGames))
	}
...
func skillsSource(b *opendota.SkillBuild, hero, role string, w sourceWords) Source {
	...
	if len(b.Order) == 0 {
		src.From = w.f("No pro skill order: pros played %[1]s in fewer than %[3]d games over the last %[2]d days, so the skill point reminder just says to level up.",
			...
			hero, opendota.ProDays, opendota.MinProGames)
		return src
	}
	...
	if b.Position > 0 {
		src.From = w.f("OpenDota: skill orders from the %d most recent %s on %s as %s, over the last %d days.",
			...
			b.Games, games, hero, w.position(b.Position), opendota.ProDays)
	} else {
		src.From = w.f("OpenDota: skill orders from the %d most recent %s on %s in every position, over the last %d days, because too few were as %s.",
			...
			b.Games, games, hero, opendota.ProDays, dota.RoleName(role, w.lang))
	}
...
func sources(build *opendota.Build, skills *opendota.SkillBuild, t Targets, timings dota.Timings, hero, role, lang string) []Source {
	w := sourceWords{lang}
	var out []Source
	if build != nil {
