...
func (c *Ctx) nextItemCost() (string, int, bool) {
	...
	return next.DName, opendota.RemainingCost(next.Name, held, items), true
}
...
func (c *Ctx) nextItemGoal() (ItemGoal, bool) {
	...
	owned := opendota.OwnedClosure(heldNames(c.S), items)
...
func (c *Ctx) itemGoalGold() int {
	...
	return max(opendota.RemainingCost(g.Item, heldNames(c.S), c.items())-c.S.Player.Gold, 0)
}
