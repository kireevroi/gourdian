...
func TestSourcesSayWhereAdviceComesFrom(t *testing.T) {
	...
	skills := []*opendota.SkillBuild{{Position: 2, Games: 235, Won: true, Order: []string{"a"}}, {Games: 19, Won: true, Order: []string{"a"}}, {Position: 2, Games: 15, Order: []string{"a"}}, {}}
	targets := []Targets{
...
func TestRussianSourcesReadWell(t *testing.T) {
	w := sourceWords{"ru"}
	got := buildSource(&opendota.Build{Position: 2, Games: 235, Won: true}, "Storm Spirit", dota.Mid, w).From
	...
	got = buildSource(&opendota.Build{Games: 97, Won: true}, "Pudge", dota.Mid, w).From
