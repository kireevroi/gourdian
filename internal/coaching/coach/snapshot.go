...
func (e *Engine) Snapshot(set config.Settings) Snapshot {
	...
	var items map[string]opendota.ItemInfo
	var build *opendota.Build
	if e.data != nil {
	...
	if build != nil && items != nil {
		...
		for i, it := range build.Items {
			...
			if !v.Owned {
				v.Remaining = opendota.RemainingCost(it.Name, held, items)
			}
	...
	if items != nil {
		held := heldNames(s)
		owned := opendota.OwnedClosure(held, items)
		for _, g := range targets.Items {
			v := ItemGoalView{ItemGoal: g, Owned: owned[g.Item]}
			if !v.Owned {
				v.Remaining = opendota.RemainingCost(g.Item, held, items)
			}
			if e.match != nil {
	...
	var skills *opendota.SkillBuild
	if e.data != nil {
