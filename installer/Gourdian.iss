...
begin
  { Ask a running copy to quit cleanly so its exe can be replaced, under either name. }
  QuitRunning(ExpandConstant('{app}\{#AppExe}'));
  QuitRunning(ExpandConstant('{app}\{#LegacyExe}'));
  Result := '';
end;
